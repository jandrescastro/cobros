      ...
      <header className="mb-5 space-y-2.5">
        <div className="flex flex-col gap-3">
          <div className="min-w-0">
            ...
            <h1 className="font-display text-[1.5rem] font-extrabold uppercase leading-[0.95] tracking-[0.01em] sm:text-[1.8rem]">
              {title}
            </h1>
          </div>
          <div className="flex justify-start">{actions ?? <SessionToolbar role={role} name={viewerName} />}</div>
        </div>
        {description ? <p className="max-w-[34ch] text-[0.88rem] leading-5 text-slate">{description}</p> : null}
      </header>
